...
describe("origami timeline", () => {
  ...
  it("navigates to strictly adjacent boundaries", () => {
    ...
    expect(
      adjacentBoundary(document, document.metadata.duration, -1),
    ).toBeLessThan(document.metadata.duration);
  });

  it("interpolates a fold without changing source geometry", () => {
    if (source.data.kind !== "polygon" || !("targetPoints" in fold))
      throw new Error("expected polygon fold");
    const halfway = evaluateOrigamiTimeline(
      document,
      (fold.start + fold.end) / 2,
    )[source.id];
    expect(halfway.folding).toBe(true);
    expect(halfway.points?.[0].x).toBeCloseTo(
      (source.data.points[0].x + fold.targetPoints[0].x) / 2,
    );
  ...
  it("lands exactly on computed target geometry", () => {
    ...
    expect(
      evaluateOrigamiTimeline(document, fold.end)[fold.objectId].points,
  ...
  it("handles zero-duration actions deterministically", () => {
    ...
    expect(
      evaluateOrigamiTimeline(copy, 0.49)[copy.revealActions[0].objectId]
        .opacity,
    ).toBe(0);
    expect(
      evaluateOrigamiTimeline(copy, 0.5)[copy.revealActions[0].objectId]
        .opacity,
    ).toBe(1);
  });
});
