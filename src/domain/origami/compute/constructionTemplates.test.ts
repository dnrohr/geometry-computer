...
describe("origami construction templates", () => {
  it.each([
    ...
    [
      {
        operation: "add",
        left: rationalNumber(2),
        right: rationalNumber(3),
      } as const,
      5,
    ],
    [
      {
        operation: "subtract",
        left: rationalNumber(3),
        right: rationalNumber(2),
      } as const,
      1,
    ],
    [
      {
        operation: "multiply",
        left: rationalNumber(2),
        right: rationalNumber(rational(3, 2)),
      } as const,
      3,
    ],
  ...
  it("rejects layouts outside the safe paper region", () => {
    expect(() =>
      instantiateConstructionTemplate({
        operation: "add",
        left: rationalNumber(80),
        right: rationalNumber(80),
      }),
    ).toThrow(/paper bounds/i);
  });
});
