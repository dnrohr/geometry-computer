...
import { emptyOrigamiSession, appendSessionFold, type OrigamiSession } from "../session";
import { exactText } from "../algebra/origamiNumber";
import { textR } from "../algebra/rational";
import { solveAxiom } from "../axioms";
import { analyzeOrigamiExpression, type ExpressionAnalysis } from "./analyzeExpression";
import {
...
const children = (expr: Expr): Expr[] => {
  switch (expr.kind) {
    ...
    case "sqrt": case "cbrt": return [expr.value];
    case "pow": return [expr.base];
    case "cubicRoot": return expr.coefficients;
    default: return [expr.left, expr.right];
  }
};

const rationalCoefficient = (value: OrigamiNumber): bigint[] => {
  if (value.polynomial.length !== 2)
    throw new OrigamiPlanningError("Cubic coefficients must be rational.", "NON_RATIONAL_COEFFICIENT");
...
export function planOrigamiExpression(
  ...
  if (!analysis.value)
    throw new OrigamiPlanningError(analysis.reason, analysis.diagnostic?.code ?? "ANALYSIS_FAILED");
  const analyzedByExpression = new Map(analysis.nodes.map((node) => [node.expression, node]));
  const templates: ConstructionTemplate[] = [];
  const nodes: PlannedExpressionNode[] = [];
  const visited = new Set<string>();
  let session = emptyOrigamiSession(`Origami compute: ${formatExpression(expression)}`);
  ...
  const visit = (expr: Expr) => {
    ...
    const dependencyIds = children(expr).map((child) => analyzedByExpression.get(formatExpression(child))!.id);
    let request: TemplateRequest | undefined;
    const valueOf = (child: Expr) => analyzedByExpression.get(formatExpression(child))!.value!;
    switch (expr.kind) {
      case "const": case "var": break;
      case "add": request = { operation: "add", left: valueOf(expr.left), right: valueOf(expr.right) }; break;
      case "sub": request = { operation: "subtract", left: valueOf(expr.left), right: valueOf(expr.right) }; break;
      case "mul": request = { operation: "multiply", left: valueOf(expr.left), right: valueOf(expr.right) }; break;
      case "div": request = { operation: "divide", left: valueOf(expr.left), right: valueOf(expr.right) }; break;
      case "sqrt": request = { operation: "square-root", value: valueOf(expr.value) }; break;
      case "cbrt": request = { operation: "cube-root", value: valueOf(expr.value) }; break;
      case "pow": {
        if (expr.exponent === 0) request = { operation: "unit" };
        else if (expr.exponent === 2) request = { operation: "multiply", left: valueOf(expr.base), right: valueOf(expr.base) };
        else throw new OrigamiPlanningError("The fold planner currently expands only powers 0 and 2.", "PLANNER_POWER_LIMIT");
        break;
      }
      case "cubicRoot": {
        const rationals = expr.coefficients.map((coefficient) => rationalCoefficient(valueOf(coefficient)));
        const denominator = rationals.reduce((product, pair) => product * pair[1], 1n);
        const integer = rationals.map(([numerator, divisor]) => numerator * (denominator / divisor));
        request = {
    ...
    if (!request) {
      nodes.push({ expressionNodeId: analyzed.id, expression: key, kind: expr.kind, dependencyIds, foldIds: [] });
      return;
    }
    ...
    for (const fold of template.folds) {
      ...
      const rejected = new Map(fold.rejectedCandidates.map((candidate) => [candidate.index, candidate.reason]));
      ...
      steps[stepIndex] = {
        ...
        provenance: {
          ...
          creaseObjectIds: steps[stepIndex].document.objects.filter(({ kind }) => kind === "crease").map(({ id }) => id),
    ...
    nodes.push({ expressionNodeId: analyzed.id, expression: key, kind: expr.kind, dependencyIds, templateId, foldIds });
  };
