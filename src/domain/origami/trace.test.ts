...
describe("origami trace", () => {
  it("builds ordered domain-backed phases", () => {
    ...
    expect(phases.map(({ time }) => time)).toEqual(
      [...phases.map(({ time }) => time)].sort((a, b) => a - b),
    );
    expect(phases.find(({ id }) => id === "fold")?.summary).toContain(
      example.movingSide,
    );
  ...
  it("selects the active phase from timeline time", () => {
    ...
    expect(
      activeTracePhase(phases, phases.find(({ id }) => id === "unfold")!.time)
        .id,
    ).toBe("unfold");
  });
});
