...
describe("3D hinge evaluation", () => {
  ...
  it("starts at the canonical source and settles exactly on the flat target", () => {
    if (object.data.kind !== "polygon" || !("targetPoints" in fold)) throw new Error("expected fold polygon");
    expect(evaluateHingeObject(document, object.id, fold.start)?.points.map(({ x, y }) => ({ x, y }))).toEqual(object.data.points);
    expect(evaluateHingeObject(document, object.id, fold.end)?.points.map(({ x, y }) => ({ x, y }))).toEqual(fold.targetPoints);
  });

  it("lifts moving vertices out of plane at the midpoint while the crease stays fixed", () => {
    const midpoint = evaluateHingeObject(document, object.id, (fold.start + fold.end) / 2)!;
    ...
    const dx = creaseData.end.x - creaseData.start.x; const dy = creaseData.end.y - creaseData.start.y;
    const fixedIndex = object.data.points.findIndex(({ x, y }) => Math.abs((x - creaseData.start.x) * dy - (y - creaseData.start.y) * dx) < 1e-7);
