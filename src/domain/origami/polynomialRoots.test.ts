...
describe("certified-degree real root isolation", () => {
  it.each([
    ...
    (polynomial, expected) => {
      ...
      [...new Set(expected)].forEach((value, index) =>
        expect(roots[index]).toBeCloseTo(value, 8),
      );
      roots.forEach((root) =>
        expect(Math.abs(evaluatePolynomial(polynomial, root))).toBeLessThan(
          1e-8,
        ),
      );
    },
  ...
  it("isolates higher-degree roots used by composed algebraic operations", () => {
    realPolynomialRoots([4, 0, -5, 0, 1]).forEach((root, index) =>
      expect(root).toBeCloseTo([-2, -1, 1, 2][index], 8),
    );
  });
});
