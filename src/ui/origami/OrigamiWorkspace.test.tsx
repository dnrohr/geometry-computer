...
describe("OrigamiWorkspace authoring history", () => {
  it("computes, undoes, and redoes a guided fold", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "My guided fold" }),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Undo" }));
    expect(
      screen.getByRole("heading", { name: "Edge bisection" }),
    ).toBeInTheDocument();
  ...
  it("retains the last valid construction after an invalid request", () => {
    render(<OrigamiWorkspace />);
    fireEvent.change(screen.getByLabelText("Target X"), {
      target: { value: "2" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Compute fold" }));
    expect(screen.getByRole("alert")).toHaveTextContent(
      "last valid construction is unchanged",
    );
    expect(
      screen.getByRole("heading", { name: "Edge bisection" }),
    ).toBeInTheDocument();
  ...
  it("loads and navigates the three-fold reference session", () => {
    render(<OrigamiWorkspace />);
    fireEvent.click(
      screen.getByRole("button", { name: "Load three-fold reference" }),
    );
    expect(
      screen.getByRole("heading", { name: "Fold left to right" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("slider", { name: "Session timeline" }),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Next fold" }));
    expect(
      screen.getByRole("heading", { name: "Fold across the center" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Save session JSON" }),
    ).toBeEnabled();
  ...
  it("reports malformed session input without discarding the current view", () => {
    ...
    fireEvent.change(screen.getByLabelText("Saved session JSON"), {
      target: { value: "{}" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Load session JSON" }));
    expect(screen.getByRole("alert")).toHaveTextContent(
      /malformed origami session/i,
    );
    expect(
      screen.getByRole("heading", { name: "Edge bisection" }),
    ).toBeInTheDocument();
  ...
  it("preserves timeline state while switching between 2D and 3D", () => {
    ...
    const saved = (
      screen.getByRole("slider", { name: "Timeline" }) as HTMLInputElement
    ).value;
