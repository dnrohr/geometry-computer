...
describe("OrigamiCanvas", () => {
  ...
  it("renders distinct source, midpoint, and target geometry", () => {
    const { rerender } = render(
      <OrigamiCanvas document={document} time={fold.start} />,
    );
    const moving = screen.getByRole("button", {
      name: new RegExp(fold.objectId),
    });
    const source = moving.querySelector("polygon")!.getAttribute("points");
    rerender(
      <OrigamiCanvas document={document} time={(fold.start + fold.end) / 2} />,
    );
  ...
  it("renders timed labels and stable view box", () => {
    const { container } = render(
      <OrigamiCanvas document={document} time={document.metadata.duration} />,
    );
  ...
  it("reports paper coordinates for direct point selection", () => {
    ...
    expect(onPaperPoint).toHaveBeenCalledWith(
