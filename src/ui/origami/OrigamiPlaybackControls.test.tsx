...
const playback = (playing = false) => ({
  duration: 5, time: 1.25, playing, playbackRate: 1,
  setPlaybackRate: vi.fn(), setTime: vi.fn(), play: vi.fn(), pause: vi.fn(),
  restart: vi.fn(), previous: vi.fn(), next: vi.fn(),
});
...
describe("OrigamiPlaybackControls", () => {
  it("exposes timeline and action navigation", () => {
    ...
    fireEvent.change(screen.getByRole("slider", { name: "Timeline" }), { target: { value: "3.5" } });
