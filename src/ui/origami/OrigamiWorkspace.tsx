...
import {
  compileGuidedFold,
  type GuidedFoldRequest,
} from "../../domain/origami/guidedFold";
import {
  appendSessionFold,
  emptyOrigamiSession,
  parseOrigamiSession,
...
} from "../../domain/origami/session";
import {
  activeTracePhase,
  buildOrigamiTrace,
  type OrigamiTracePhase,
} from "../../domain/origami/trace";
import { ManimExportPanel } from "../../render/manim/ManimExportPanel";
import {
  OrigamiAuthorPanel,
  type OrigamiAuthorHandle,
} from "./OrigamiAuthorPanel";
...
type HistoryEntry = ReturnType<typeof compileGuidedFold> & {
...
export function OrigamiWorkspace() {
  ...
  const builtInDocument = useMemo(
    () => compileOrigamiExample(builtInExample),
    [builtInExample],
  );
  ...
  const compileRequest = (request: GuidedFoldRequest) => {
    try {
      const next = { request, ...compileGuidedFold(request) };
      const retained = history.slice(0, historyIndex + 1);
      setHistory([...retained, next]);
      setHistoryIndex(retained.length);
      setShowSession(false);
      setSelectedObjectId(undefined);
      setAuthorError(undefined);
      localPlayback.restart();
      ...
      setAuthorError(
        reason instanceof Error
          ? reason.message
          : "The requested fold could not be computed.",
      );
    }
  };
  const appendCurrent = () => {
    if (!guided) return;
    try {
      const next = appendSessionFold(session, guided.request);
      sessionPlayback.setTime(session.duration);
      setSession(next);
      setShowSession(true);
      setAuthorError(undefined);
    } catch (reason) {
      setAuthorError(
        `This fold cannot be added to the current paper state: ${reason instanceof Error ? reason.message : String(reason)}`,
      );
    }
  };
  const loadSession = (source: string) => {
    try {
      const next = parseOrigamiSession(source);
      setSession(next);
  ...
  const loadReference = () => {
    try {
      setSession(threeFoldReferenceSession());
      sessionPlayback.restart();
      setShowSession(true);
      setAuthorError(undefined);
    } catch (reason) {
  ...
  const selectComputeNode = (plan: OrigamiComputePlan, nodeId: string) => {
    const node = plan.nodes.find(
      ({ expressionNodeId }) => expressionNodeId === nodeId,
    );
  ...
  return (
    <>
      <OrigamiComputePanel
        onCompile={loadComputePlan}
        onSelectNode={selectComputeNode}
      />
      <section className="origami-intro">
        <p className="section-label">Computed paper folding</p>
        <h2>Choose a fold</h2>
        <div
          className="origami-examples"
          role="list"
          aria-label="Origami examples"
        >
          {origamiExamples.map((item) => (
            <button
              key={item.id}
              type="button"
              className={item.id === selectedId && !guided ? "active" : ""}
      ...
      <OrigamiSessionPanel
        ...
        activeStepId={sessionActive ? position?.step.id : undefined}
        canAppend={Boolean(guided)}
        onAppend={appendCurrent}
        onLoadReference={loadReference}
        onExport={() =>
          downloadText(
            "origami-session.json",
            serializeOrigamiSession(session),
            "application/json",
          )
        }
        onImport={loadSession}
        onReset={() => {
          sessionPlayback.restart();
          setSession(emptyOrigamiSession());
          setShowSession(false);
        }}
      />
      <section className="origami-preview" aria-labelledby="origami-title">
        <div>
          <p className="section-label">
            {sessionActive
              ? `Session fold ${position!.stepIndex + 1} of ${session.steps.length}`
              : "Current fold"}
          </p>
          <h2 id="origami-title">{example.title}</h2>
          <p>{example.description}</p>
          <p className="origami-instruction">
            Move the <strong>{example.movingSide}</strong> side across the
            computed crease.
          </p>
          <dl className="origami-provenance">
            <dt>Operation</dt>
            <dd>{example.operation.replaceAll("-", " ")}</dd>
            <dt>Branch</dt>
            <dd>{example.branch ?? "unique"}</dd>
            <dt>Assumptions</dt>
            <dd>{example.assumptions.join(" ")}</dd>
            <dt>Tolerance</dt>
            <dd>
              1 × 10<sup>−9</sup>
            </dd>
        ...
        <div className="origami-canvas-stack">
          <div
            className="origami-view-switch"
            role="group"
            aria-label="Fold view"
          >
            <button
              type="button"
          ...
          {viewMode === "2d" ? (
            <OrigamiCanvas
              ...
              onPaperPoint={(point) =>
                authorRef.current?.applyCanvasPoint(point)
              }
            />
          ) : (
            <Origami3DCanvas
          ...
          {!sessionActive && (
            <OrigamiPlaybackControls playback={localPlayback} />
          )}
        </div>
      </section>
      <div className="origami-details">
        <OrigamiTracePanel
          phases={phases}
          activeId={activePhase.id}
          onSelect={(phase) =>
            sessionActive
              ? sessionPlayback.setTime(position!.step.start + phase.time)
              : localPlayback.setTime(phase.time)
          }
          onHover={setHoveredPhase}
        />
        <OrigamiObjectInspector
      ...
      <ManimExportPanel
        document={document}
        session={sessionActive ? session : undefined}
      />
    </>
  );
}
