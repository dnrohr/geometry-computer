...
export function OrigamiCanvas({
  ...
  const timeline = useMemo(
    () => evaluateOrigamiTimeline(document, time),
    [document, time],
  );
    ...
    () =>
      new Set(
        document.revealActions
          .filter(
            ({ animation }) => animation === "fold" || animation === "unfold",
          )
          .map(({ objectId }) => objectId),
      ),
    [document],
  );
  const ordered = [...document.objects].sort(
    (a, b) => (timeline[a.id].layer ?? 0) - (timeline[b.id].layer ?? 0),
  );
  ...
  const pickPaperPoint = (event: MouseEvent<SVGSVGElement>) => {
    ...
    onPaperPoint({
      x: x + ((event.clientX - bounds.left) / bounds.width) * width,
      y: y + ((event.clientY - bounds.top) / bounds.height) * height,
    });
  };
  return (
    <svg
      ...
      role="img"
      aria-label={`${document.metadata.title} fold diagram`}
      onClick={pickPaperPoint}
    >
      <defs>
        <marker
          id="origami-arrowhead"
          markerWidth="5"
          markerHeight="5"
          refX="4"
          refY="2.5"
          orient="auto"
        >
          <path d="M0,0 L5,2.5 L0,5 z" fill={colors.reference} />
        </marker>
      </defs>
      {ordered.map((object) => {
        ...
        const common = {
          ...
          tabIndex: 0,
          role: "button",
          "aria-label": objectName(object),
          "data-object-id": object.id,
          "data-motion": state.folding
            ? "folding"
            : movingIds.has(object.id)
              ? "moving"
              : "stationary",
          onClick: () => activate(object.id),
          onKeyDown: (event: KeyboardEvent<SVGGElement>) =>
            keyboardActivate(event, object.id),
          ...
          <g key={object.id} {...common}>
            {data.kind === "polygon" && (
              <polygon
                points={(state.points ?? data.points)
                  .map(({ x, y }) => `${x},${y}`)
                  .join(" ")}
                fill={state.side === "back" ? "#8bb6c7" : colors.paper}
            ...
            {(data.kind === "crease" ||
              ...
              <line
                ...
                stroke={colors[object.role] ?? colors.reference}
                strokeWidth={data.kind === "crease" ? "0.1" : "0.08"}
                strokeDasharray={
                  data.kind === "crease"
                    ? "0.04 0.03"
                ...
                markerEnd={
                  data.kind === "arrow" ? "url(#origami-arrowhead)" : undefined
                }
              />
            )}
            {data.kind === "point" && (
