...
The parser produces a precedence-aware AST. Recursive compilation first compiles operands, reuses formatted repeated subexpressions, then emits the parent operation. `^2` normalizes to the square operation; the simplification hook can advise on all-constant subtrees, identities, nearby constant combinations, self-subtraction, and first-power reuse without changing the compiler input. The reuse optimization summary is also advisory: it counts `reuse-length` transfers and duplicate source phases avoided, while the compiled operation order remains the source of truth. The original display expression is retained independently from an example's manually supplied simplified form.
