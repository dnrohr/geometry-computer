...
- F7.18 adds a plan-verification row to the collapsed diagnostics pane. The row
  reports whether generated nodes, operations, phases, length transfers, and
  result extraction are internally connected before future mathematical
  fold-validity checks are attempted.
...
- F6.7 adds `Export function script` to the collapsed export/paper-style section.
  ...
  lines.
- F6.8 adds `Import function script` beside the JSON replay input. Script replay
...
- F7.15 and F7.16 add optional simplification hints for local algebra
  ...
  expression, suggested replacement, and reason while leaving the rendered fold
  sequence tied to the original compiled plan.
